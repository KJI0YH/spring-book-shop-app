    ...
    $(function () {
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="' + book.slug + '">' + '       <img src="' + book.image + '">\n' + '</a>' +
                    ...
                    '<div class="Card-content">' + '<strong class="Card-title"><a href="/books/' + book.slug + '">' + book.title + '</a></strong>' +
                    '<div class="Card-description"><span>' + book.authors[0] + ' </span>' + (book.authors.length > 1 ? '<span>and others</span>' : '') + '</div>' +
                    '<div class="Card-cost">' + (book.discountPrice ? '<span class="Card-priceOld">₽' + book.price / 100 + '</span>' : '') + '<span class="Card-price">₽' + book.discountPrice / 100 + '</span>' + '</div>' + '</div>' + '</div>';
            }
            return {
                init: function () {
                                ...
                                postData('/login', $this.data('send'), function (result) {
                                    if (result != null && result.result) {
                                        ...
                                        if (result) {
                                            ...
                                            $thisStep.find('.form-input:not(.form-input_hide)[type="text"]').addClass('form-input_error').after('<div class="form-error">' + "Пользователь не найдет или неверный логин\/пароль" + '</div>');
                                        }
                                    }
                                })
                            } else {
                                ...
                                if (result.result) {
                                    ...
                                    if ($cart.length) {
                                        ...
                                        $cart.find('.Cart-product').each(function () {
                                            var $this = $(this);
                                            price += parseFloat($this.find('.Cart-price:not(.Cart-price_old)').text().substr(1));
                                            priceOld += parseFloat($this.find('.Cart-price_old').text().substr(1));
        ...
        var Profile = function () {
            ...
            return {
                init: function () {
                    ...
                    var confirmChangeEmailButton = $('#confirmChangeEmailButton');
                    confirmChangeEmailButton.click(function () {
                        var newEmail = $("#mail").val();
                        Login().postData('/profile/change/email', {contact: newEmail}, function (result) {
                        });
                    });
                    var confirmChangePhoneButton = $('#confirmChangePhoneButton');
                    confirmChangePhoneButton.click(function () {
                        var newPhone = $("#phone").val();
                        Login().postData('/profile/change/phone', {contact: newPhone}, function (result) {
                        });
                    })
                }
            };
                ...
                init: function () {
                    $('form.Topup').on('submit', function (e) {
                        ...
                        Login().postData('/payment', {
                            hash: $this.data('sendhash').toString(),
        ...
        var Transactions = function () {
            var transactionTemplate = function (transaction) {
                ...
                transactionValue += Math.floor(transaction.value / 100) + '.' + transaction.value % 100 + 'р.';
                var timeValue = String(date.getDate()).padStart(2, '0') + '.' +
                    String(date.getMonth() + 1).padStart(2, '0') + '.' +
                    date.getFullYear() + ' ' +
                    String(date.getHours()).padStart(2, '0') + ':' +
                    String(date.getMinutes()).padStart(2, '0') + ':' +
                    String(date.getSeconds()).padStart(2, '0');
                var descriptionValue = transaction.description;
                if (transaction.book != null) {
                    descriptionValue = '<a href="/books/' + transaction.book.slug +
            ...
            function getData(address, data) {
                ...
                $.ajax({
                    ...
                    complete: function (result) {
                        if (result.status === 200) {
                            var data = '';
                            if (result.responseJSON.count < 50) {
            ...
            $('.Transactions-time').on('click', function (e) {
                var getButton = $('.Transactions-get');
                if (getButton.data('transactionsort') == 'desc') {
                    getButton.data('transactionsort', 'asc');
                } else {
                    getButton.data('transactionsort', 'desc');
                }
                getButton.data('transactionoffset', 0);
                getButton.show();
                $('.Transactions-body').empty();
                getData('/api/transactions', {
                    sort: getButton.data('transactionsort'),
                    offset: function () {
                        let oldOffset = getButton.data('transactionoffset');
                        getButton.data('transactionoffset', oldOffset++);
                        return getButton.data('transactionoffset');
                    },
                    limit: getButton.data('transactionlimit')
                });
            });
                ...
                init: function () {
                    $('.Transactions-get').on('click', function (e) {
                        ...
                        getData('/api/transactions', {
                            ...
                            offset: function () {
                                let oldOffset = $this.data('transactionoffset');
